...
一个 Claude Code 插件，为 AI 编码引入自动 peer review —— 一个 AI 写，另一个 AI 审。
...
然后运行 `/workflow:setup` 安装依赖、选择审查模型。

## 命令

| 命令 | 说明 |
|------|------|
| `/workflow:setup` | 安装依赖、选择模型、配置权限 |
| `/workflow:workflow <任务> <执行者>` | 普通模式 —— 先讨论再自动审查 |
| `/workflow:workflow-yolo <任务> <执行者>` | YOLO 模式 —— 全自动，只确认提交 |
| `/workflow:workflow-issue <看板URL> [issue编号]` | Issue 模式 —— 从 GitHub Issue 获取任务 |
...
- **普通模式** —— 你先参与方案讨论，然后 AI 双方自动完成方案和代码 review
...
使用时传入看板链接：
```
/workflow:workflow-issue https://github.com/orgs/your-org/projects/3 106
```
...
身份标记（让人一眼看出谁说的）：
- 🧠 **Claude Code** —— 方案、实现
- 🤖 **Cursor Agent** —— 审查意见（自动同步）
- 🔧 **Workflow** —— 状态流转

不需要固定配置——每次对话传入看板链接即可，不同会话可以用不同看板。

## 审查者配置

审查者（Cursor Agent）的模型和 prompt 配置在 `agents/cursor.md`，由 `/workflow:setup` 生成。
